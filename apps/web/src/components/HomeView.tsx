...
export function HomeView({
  ...
  useEffect(() => {
    try {
      const studioBrief = sessionStorage.getItem('juh-design:brief');
      if (!studioBrief) return;
      setPrompt((existing) => existing.trim() ? `${existing}\n\n${studioBrief}` : studioBrief);
      setPromptEditedByUser(true);
      sessionStorage.removeItem('juh-design:brief');
    } catch { /* The studio also offers a downloadable briefing. */ }
  }, []);
  ...
  return (
    <div
      ...
      ref={homeViewRef}
    >
      <a href="/estudio" style={{ alignSelf: 'flex-end', padding: '12px 24px', position: 'relative', zIndex: 1 }}>Juh Design · Estúdio criativo ↗</a>
